...
class ProviderQuerier:
    ...
    def QueryProviders(self, statusEnum: int):
        ...
        while(next_key):
            if(next_key == 0x01):
                r = self.__stub.QueryProviders(sentinel_provider_v2_querier_pb2.QueryProvidersRequest(status=statusEnum))
            else:
                next_page_req = cosmos_pagination_pb2.PageRequest(key=next_key)
                r = self.__stub.QueryProviders(sentinel_provider_v2_querier_pb2.QueryProvidersRequest(status=statusEnum, pagination=next_page_req))
